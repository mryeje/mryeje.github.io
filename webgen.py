...
def generate_html(model_id, model_data):
    ...
    function showdown(item, index) {
        ...
        // New function here
        function determineAnnotationIndexForSelectedPart(selectedPart) {
            var partNames = Object.keys(parts);
            var index = partNames.indexOf(selectedPart);
            return index; // Directly returns the index of the part in the parts object
        }
...
document.getElementById("showButton").addEventListener("click", function() {
    var selectedPart = document.getElementById("partSelect").value;
    if (selectedPart === "All Parts") {
        // Loop through all parts and show them
        Object.keys(parts).forEach(function(part) {
            parts[part].forEach(function(item) {
                api.show(item); // Assuming api.show() is the correct way to show an item
            });
        });
    } else {
        // Show only the selected part
        parts[selectedPart].forEach(function(item) {
            api.show(item); // Show each item in the selected part
        });
    }
});
...
document.getElementById('resetViewButton').addEventListener('click', function() {
    ...
    api.recenterCamera(function(err) {
        if (!err) {
            ...
            // Assuming showAllAnnotations function exists and you want to show all annotations upon reset
            showAllAnnotations(api);

            // Assuming parts is an object where each key is a part name and each value is an array of item IDs to show
            Object.keys(parts).forEach(function(part) {
                parts[part].forEach(function(item) {
                    api.show(item); // Make sure this method is supported by your API instance to show each item
                });
            });

            console.log('All parts should now be visible.');
        } else {
